...
class FullyConnectedLayer(object):
    ...
    def forward(self, input):  # 前向传播计算
        ...
        # self.output = self.weight.T.dot(self.input) + self.bias
    ...
    def backward(self, top_diff):  # 反向传播的计算
        ...
        # self.d_bias = np.dot(top_diff.T , np.ones([top_diff.shape[0] , 1]))
...
class SoftmaxLossLayer(object):
    ...
    def forward(self, input):  # 前向传播的计算
        ...
        self.prob = input_exp / np.sum(input_exp, axis=1, keepdims=True) #压缩列实现求和
        return self.prob
