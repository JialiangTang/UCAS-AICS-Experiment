...
def run_ori_power_diff_pb(ori_power_diff_pb, image):
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            start_time = time.time()
            ret =sess.run(output_tensor, feed_dict={input_tensor:[X], input_tensor_pow: input_pow})       #########input_pow:2.0
            end_time = time.time()
...
def run_numpy_pb(numpy_pb, image):
    ...
    with g.as_default():
        ...
        with tf.Session(config=config) as sess:
            ...
            input_pow = 2
            output = power_diff_numpy(input_x, input_y, input_pow).reshape(1, 256, 256, 3)       ################reshape?
