...
def power_difference_op(input_x,input_y,input_pow):
    with tf.Session() as sess:
        ...
        place_x = tf.placeholder(tf.float32, shape=input_x.shape, name='placeholder_x')
        place_y = tf.placeholder(tf.float32, shape=input_y.shape, name='placeholder_y')
        place_z = tf.placeholder(tf.float32, name='placeholder_z')
        out = tf.power_difference(place_x, place_y, place_z)
        return sess.run(out, feed_dict = {place_x:input_x, place_y:input_y, place_z:input_pow})
