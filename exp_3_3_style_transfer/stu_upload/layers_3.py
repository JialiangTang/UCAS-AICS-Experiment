...
class StyleLossLayer(object):
    ...
    def forward(self, input_layer, style_layer):
        ...
        self.gram_style = np.zeros([style_layer.shape[0], style_layer.shape[1], style_layer.shape[1]]) ## Gram 矩阵初始化：N*c_in*c_in
        ...
        # self.div = input_layer.shape[1] * input_layer.shape[1] * input_layer.shape[2] * input_layer.shape[3] * input_layer.shape[2] * input_layer.shape[3]
        # TODO： 计算风格迁移图像和目标风格图像的风格损失
        # style_diff = np.sum((self.gram_style - self.gram_input) ** 2)
        loss = np.sum((self.gram_style - self.gram_input) ** 2) / (4 * input_layer.shape[0] * input_layer.shape[1] * input_layer.shape[1] * input_layer.shape[2] * input_layer.shape[3] * input_layer.shape[2] * input_layer.shape[3])
        return loss
    def backward(self, input_layer, style_layer):
        
        bottom_diff = np.zeros([input_layer.shape[0], input_layer.shape[1], input_layer.shape[2]*input_layer.shape[3]]) # N, c_in, h*w
        for idxn in range(input_layer.shape[0]):
            # TODO： 计算风格损失的反向传播
            bottom_diff[idxn, :, :] = np.matmul((self.gram_input - self.gram_style)[idxn, :, :].T, style_layer.reshape(style_layer.shape[0], style_layer.shape[1], -1)[idxn, :, :]) / (style_layer.shape[0] * input_layer.shape[1] * input_layer.shape[1] * input_layer.shape[2] * input_layer.shape[3] * input_layer.shape[2] * input_layer.shape[3])
