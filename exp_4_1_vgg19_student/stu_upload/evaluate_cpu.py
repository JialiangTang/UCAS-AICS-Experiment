...
def net(data_path, input_image):
    ...
    for i, name in enumerate(layers):
        if name[:4] == 'conv':
            ...
            current = tf.nn.softmax(current)     ######################axis指的是？需要axis=1吗？
        elif name  == 'fc6':
            # TODO: 执行全连接层计算，计算结果存入 current
            kernels, bias = weights[i][0][0][0][0]
            current = tf.reshape(current, [-1, current.shape[1]*current.shape[2]*current.shape[3]])
            kernels = tf.reshape(kernels, [kernels.shape[0]*kernels.shape[1]*kernels.shape[2], -1])  ###############一定要用tf.reshape吗？直接np.reshape会怎么样
...
def _conv_layer(input, weights, bias):
    ...
    conv_result = tf.nn.conv2d(input, weights, 1, "SAME") #######################################问题：如果边界扩充不是1该咋办？
    conv_result = tf.nn.bias_add(conv_result, bias.flatten())  ######################要flatten?
    return conv_result
...
def load_image(path):
    ...
    image = scipy.misc.imread(path) #读取
    image = scipy.misc.imresize(image, (224, 224, 3)) #reshape一下
    image = preprocess(image, mean).reshape(1, 224, 224, 3) #preprocess一下
    return image
...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        ...
        nets = net(VGG_PATH, img_placeholder)  ################################放在sess外面是否可以？

        for i in range(10):
            start = time.time()
            # TODO: 计算 nets
            preds = sess.run(nets, feed_dict={img_placeholder:input_image}) ###############################会话和计算图的机理？
            end = time.time()
