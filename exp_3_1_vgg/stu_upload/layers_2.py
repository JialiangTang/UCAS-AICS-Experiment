...
class ConvolutionalLayer(object):
    ...
    def forward(self, input):  # 前向传播的计算
        ...
        for idxn in range(self.input.shape[0]):     #fix the num of samples
            for idxc in range(self.channel_out):    #fix the num of out channels 
                for idxh in range(height_out):      #fix the hight 
                    for idxw in range(width_out):   #fix the width
...
class MaxPoolingLayer(object):
    ...
    def forward(self, input):  # 前向传播的计算
        ...
        for idxn in range(self.input.shape[0]):
            for idxc in range(self.input.shape[1]):   #diff from before!
...
class FlattenLayer(object):
    ...
    def forward(self, input):  # 前向传播的计算
        ...
        # TODO：转换 input 维度顺序
        self.input = np.transpose(input, [0, 2, 3, 1])  #change the order
